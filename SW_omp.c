...
int main(int argc, char *argv[]) {
  ...
  for (int bl=0; bl<numBlocks; bl++){
      ...
      #pragma omp single
      {
        for (i=1; i<rowsLocal; i=sb+i){
          for (j=sum;j<(col_block[bl]+sum);j=sb+j) {
            int tid = omp_get_thread_num();
            //printf("%d: %d, %d\n", tid, i, j);

            #pragma omp task \
                    depend (in: hLocal[i-1][j+sb-1], hLocal[i+sb-1][j-1]) \
                    depend (out: hLocal[i+sb-1][j+sb-1], ) \
                    private(diag, down, right, max, iA)
            {
              tid = omp_get_thread_num();
              //printf(" inside task  %d: %d, %d\n", tid, i, j);
              int imin, imax, jmin, jmax, lmax;
              imin = i;
              jmin = j;
              imax = imin + sb;
              jmax = jmin + sb;
              ...
              for (int si=imin; si<imax; si++){
                for (int sj=jmin; sj<jmax; sj++){
                  iA = si + prevRows;
                  diag    = hLocal[si-1][sj-1] + sim[a[iA]][b[sj]];
                  down    = hLocal[si-1][sj] + DELTA;
                  right   = hLocal[si][sj-1] + DELTA;
                  max=MAX3(diag,down,right);
                  if (max <= 0)         hLocal[si][sj]=0;
                  else if (max == diag) hLocal[si][sj]=diag;
                  else if (max == down) hLocal[si][sj]=down;
                  else                  hLocal[si][sj]=right;
                  //#pragma omp  critical
                  if (max > lmax) {
                    lmax=max;
                  }
